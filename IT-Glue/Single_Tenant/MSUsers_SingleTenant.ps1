...
$Description = "Documentation for all usesrs in Microsoft"
...
Connect-MsolService -AdGraphAccessToken $aadGraphToken.AccessToken -MsGraphAccessToken $graphToken.AccessToken
    ...
    $AccountSkuIdDecodeData = @{
      ...
      "BUSINESS_VOICE_MED2_TELCO"          = "Microsoft 365 Business Voice (US)"
            ...
            $PerUserMFA = Get-MsolUser -all -TenantId $customerTenantID | Select-Object DisplayName,UserPrincipalName,@{N="MFA Status"; E={if( $_.StrongAuthenticationRequirements.State -ne $null) {$_.StrongAuthenticationRequirements.State} else { "Disabled"}}}
            ...
            $userObj = foreach($user in $userList){
              ...
              ###GET Admin Roles###
              $userRole = (Get-MsolUserRole -TenantId $customerTenantID -UserPrincipalName $user.userprincipalname).Name
              ###GET Password Config and AD Sync Status###
              $Msoline = Get-MsolUser -TenantId $customerTenantID -UserPrincipalName $user.userprincipalname | Select-object LastDirSyncTime,LastPasswordChangeTimestamp,PasswordNeverExpires
              if(!$userRole){
                $userRole = "User"
              }
